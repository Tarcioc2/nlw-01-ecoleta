...
// sempre que um estado é criado para um array ou obj é preciso informar manualmente o tipo da variável
interface Item{
    id: number;
    title: string;
    image_url: string;
...
const CreatePoint = () =>{
    ...
    useEffect(() => {
        api.get('collectPoints-all').then(response => {
    ...
    useEffect(() =>{
        //carregar cidades toda vez que a UF selecionada mudar
        if(selectedUf === '0'){
            return;
        }
    ...
    async function handleSubmit(event: FormEvent){
        ...
        const filteredPoints = collectPoints.filter(collectPoint => (collectPoint.name === name || collectPoint.email === email ||
